<template>
  <div class="outer">
    <div class="academy">
      <span>学院/专业：</span>
      <el-select @blur="updateVal" v-model="pro" class="m-2" placeholder="Select">
        <el-option v-for="item in professional" :key="item.value" :label="item.label" :value="item.value"/>
      </el-select>
    </div>
    <div class="grade">
      <span>年级：</span>
      <el-select @blur="updateVal" v-model="gra" class="m-2" placeholder="Select">
...
export default defineComponent({
  ...
  setup (props, context) {
    ...
    const updateVal = () => {
      context.emit('update', { grade: +grade.value, profess: professional.value })
    }
    const addNew = () => {
      console.log(111)
    }
